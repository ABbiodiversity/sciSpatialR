...
The reference-system block above is already wired up, giving the
`crs`, `crs_name`, `datum`, and `vertical_crs` columns:
...
No readme on the share carries the reference-system block yet, so
`crs` is `NA` for every catalogued layer until they are backfilled.
Several readmes also leave the bounding coordinates blank, which
excludes them from `extent` filters. Both could also be filled from
the file headers with `terra::crs()` and `terra::ext()`.
