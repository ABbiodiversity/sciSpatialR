...
missing. A theme with no readme of its own also still appears, with
no description, so an undocumented theme folder stays visible rather
...
Thirty-seven columns is too wide to print whole; a few of them give
...
# Format the numeric columns by hand rather than through kable's
# format.args, which would also group the digits of the years.
...
tbl <- data.frame(
  ...
  year             = as.character(snapshot$year),
  "res (m)"        = num(snapshot$resolution_m, 2),
  type             = snapshot$data_type,
  "size (MB)"      = num(snapshot$size_mb, 1),
  check.names      = FALSE,
  stringsAsFactors = FALSE
)

# Missing fields print as empty cells rather than as "NA".
